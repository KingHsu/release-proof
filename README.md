...
要求 Python 3.11 和 Git。以下命令让虚拟环境和 pip 缓存都位于 D 盘：

```powershell
$PythonExe = 'D:\codexs\app-runtime\Python311\python.exe'
cd D:\codexs\workspaces\release-proof
$env:PIP_CACHE_DIR = 'D:\codexs\cache\pip'
$env:TEMP = 'D:\codexs\temp'
$env:TMP = 'D:\codexs\temp'
& $PythonExe -m venv .venv
.\.venv\Scripts\python.exe -m pip install -U pip
.\.venv\Scripts\python.exe -m pip install -e ".[dev,mcp]"
```

离线检查不会调用 DeepSeek 或 GitHub：

```powershell
.\.venv\Scripts\release-proof.exe doctor
...
先创建一个只含公开演示内容的本地 Git 仓库和 JUnit 报告：
...
```powershell
.\.venv\Scripts\release-proof.exe analyze `
  runtime\demo-repo `
  --base HEAD~1 `
  --head HEAD `
...
不提供报告时，工作流会返回 `awaiting_input`，并明确请求 JUnit/coverage/CI 材料。也可以用 `resume` 补充：

```powershell
.\.venv\Scripts\release-proof.exe resume <RUN_ID> `
  --report reports\junit.xml
```

只有显式传入 `--continue-without-reports`，系统才会继续生成证据不完整的保守报告。
...
```powershell
.\.venv\Scripts\release-proof.exe serve --host 127.0.0.1 --port 8002
```
...
```powershell
.\.venv\Scripts\streamlit.exe run src\release_proof\ui.py --server.port 8502
```
...
```powershell
.\.venv\Scripts\release-proof.exe eval
```
...
所有测试和默认演示均为 offline。若要手动验证 DeepSeek Anthropic endpoint，在本机复制 `.env.example` 为 `.env` 并填入：

```dotenv
RELEASE_PROOF_OFFLINE=false
DEEPSEEK_API_KEY=仅本机填写
...
`deepseek-v4-pro` 本身就是 1M 上下文。不要追加 `[1m]`：官方兼容层会把不支持的模型名自动映射为 Flash，名称不报错也不能证明实际使用了 Pro。

先做一个最多 16 输出 Token 的文本连通检查，再验证 forced structured tool：

```powershell
.\.venv\Scripts\python.exe scripts\smoke_text.py
.\.venv\Scripts\python.exe scripts\smoke_llm.py
```

`.env` 已被 Git 忽略。不要把 Key 发到聊天、截图、日志或 GitHub。设置 `RELEASE_PROOF_OFFLINE=false` 后，工作流会先调用一次 DeepSeek 结构化验收提取，再按风险域调用在线专家；单路由串行，多路由才并行。所有响应通过一个 forced `submit_structured_response` tool 的 `input_schema` 校验，提示版本、模型、token 和实际 Skill 都进入报告或 trace；证据引用仍由确定性 validator 复核，最终结论仍由 policy gate 生成。调用次数在请求前消费共享预算，超限会明确记录并降级到确定性实现。

只验证一次真实 structured tool（不输出 API Key）：

```powershell
.\.venv\Scripts\python.exe scripts\smoke_llm.py
```

该 smoke 成功时仅输出 model、输入/输出 token、`tool_use=true` 和 schema 校验状态。它不是完整仓库分析，也不会触发多专家。
